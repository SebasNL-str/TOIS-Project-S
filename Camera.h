...
public:
    ...
    void ProcessKeyboard(Camera_Movement direction, float deltaTime) {
        ...
        // Nueva lógica para subir y bajar
        if (direction == UP)
            Position += WorldUp * velocity; // Sube usando el vector vertical global (0,1,0)
        if (direction == DOWN)
            Position -= WorldUp * velocity; // Baja hacia el suelo

        // Descomenta la siguiente línea si quieres un movimiento tipo FPS clásico para el cementerio 
        // donde la cámara no "vuele" al mirar arriba y presionar avanzar:
